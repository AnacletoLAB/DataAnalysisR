main_clinical_data_analysis <- function(nIter = 5, ntree = 500, num_folds = 10, num.inner.folds = 7, num.boruta.folds = 5, strDir = '_covnet_score',  
  ...
  strdata = '_covnet_score'
  dirSw = file.path('C:', 'DATI', 'Policlinico-Humanitas-COVIRT', 'Andrea_RXCovid', 'MATLAB_CODE', paste('data', strDir, sep =""), 'Rcode_clinicalDataNew')
  setwd(dirSw)
  ...
  method_imputation = c( "miceRF", "missForest")
  num_imputations = 25
  thrConf = 0.05
  ...
  write.csv(mega_df, file = file.path(nameResults, "importances_RF_glm.csv"))
  ...
  risk.models = factor(c("RF", "AT", "Glm"), levels = c("RF", "AT", "Glm"))
  ...
  for (nmethod in 1: length(method_imputation)){
    for (nimp in 1: num_imputations){
      df = rbind(cbind("risk.model" = risk.models[1] , as.data.frame(t(apply(RF.results[[nmethod]][["cm.RF"]][[nimp]], 2, mean)))), 
                 ...
                 cbind("risk.model" = risk.models[3] , as.data.frame(t(apply(glm.results[[nmethod]][["cm.glmnet"]][[nimp]], 2, mean) ))))
